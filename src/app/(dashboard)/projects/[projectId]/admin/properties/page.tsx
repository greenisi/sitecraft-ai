  ...
  return (
        <div className="space-y-6">
              <div className="flex items-center justify-between">
                      <h1 className="text-2xl font-bold text-white">Properties</h1>h1>
                      <button onClick={() => { setShowForm(true); setEditing(null); setForm({ title: '', description: '', property_type: 'house', status: 'active', price: 0, address: '', city: '', state: '', zip_code: '', bedrooms: 0, bathrooms: 0, square_feet: 0 }); }}
                                  className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700">Add Property</button>button>
              </div>div>
          {showForm && (
                  <form onSubmit={handleSubmit} className="bg-gray-900 border border-gray-800 rounded-lg p-6 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                        <select value={form.property_type} onChange={e => setForm({...form, property_type: e.target.value})} className="px-3 py-2 bg-gray-800 border border-gray-700 rounded text-white">
                                                      <option value="house">House</option>option><option value="condo">Condo</option>option><option value="apartment">Apartment</option>option><option value="land">Land</option>option><option value="commercial">Commercial</option>option>
                                        </select>select>
                                        <select value={form.status} onChange={e => setForm({...form, status: e.target.value})} className="px-3 py-2 bg-gray-800 border border-gray-700 rounded text-white">
                                                      <option value="active">Active</option>option><option value="pending">Pending</option>option><option value="sold">Sold</option>option>
                                        </select>select>
              ...
              <div className="space-y-3">
                {properties.map((p) => (
                    <div key={p.id} className="bg-gray-900 border border-gray-800 rounded-lg p-4 flex items-center justify-between">
                                <div>
                                              <div className="font-medium text-white">{p.title}</div>div>
                                              <div className="text-sm text-gray-400">${p.price.toLocaleString()} - {p.bedrooms}bd/{p.bathrooms}ba - {p.city}, {p.state}</div>div>
                                              <span className={'text-xs px-2 py-0.5 rounded ' + (p.status === 'active' ? 'bg-green-600' : p.status === 'pending' ? 'bg-yellow-600' : 'bg-red-600') + ' text-white'}>{p.status}</span>span>
                                </div>div>
                                <div className="flex gap-2">
                                              <button onClick={() => { setEditing(p); setForm(p as any); setShowForm(true); }} className="px-3 py-1 text-sm bg-gray-700 text-white rounded hover:bg-gray-600">Edit</button>button>
                ...
                {properties.length === 0 && <p className="text-gray-500 text-center py-8">No properties yet. Add your first listing above.</p>p>}
              </div>div>
        </div>div>
      );
}</div>
