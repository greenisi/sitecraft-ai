  ...
  return (
        <div className="space-y-6">
              <div className="flex items-center justify-between">
                      <h1 className="text-2xl font-bold text-white">Services</h1>h1>
                      <button onClick={() => { setShowForm(true); setEditing(null); setForm({ name: '', description: '', price: 0, duration: '', is_active: true }); }}
                                  className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700">Add Service</button>button>
              </div>div>
          {showForm && (
              ...
              <div className="space-y-3">
                {services.map((s) => (
                    <div key={s.id} className="bg-gray-900 border border-gray-800 rounded-lg p-4 flex items-center justify-between">
                                <div>
                                              <div className="font-medium text-white">{s.name}</div>div>
                                              <div className="text-sm text-gray-400">${s.price} - {s.duration || 'N/A'}</div>div>
                                </div>div>
                                <div className="flex gap-2">
                                              <button onClick={() => { setEditing(s); setForm(s as any); setShowForm(true); }} className="px-3 py-1 text-sm bg-gray-700 text-white rounded hover:bg-gray-600">Edit</button>button>
                                              <button onClick={() => handleDelete(s.id)} className="px-3 py-1 text-sm bg-red-600 text-white rounded hover:bg-red-700">Delete</button>button>
                                </div>div>
                    </div>div>
                  ))}
                {services.length === 0 && <p className="text-gray-500 text-center py-8">No services yet. Add your first service above.</p>p>}
              </div>div>
        </div>div>
      );
}</div>
