...
export async function GET(
    ...
    const supabase = createClient(supabaseUrl, supabaseServiceKey);
        const { data: order, error } = await supabase.from('orders').select('order_number, status, total, currency, items, created_at').eq('project_id', projectId).eq('order_number', orderNumber).single();
