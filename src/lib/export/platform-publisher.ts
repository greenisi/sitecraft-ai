...
export async function publishToSubdomain(
  ...
  if (hasNavbar || hasFooter) {
    ...
    const formAutoWire = [
      ...
      "      wired.add(form);",
      "      // Honeypot: hidden field humans never see; bots fill it and the",
...
"        formData.forEach((v, k) => { if (k) data[k] = String(v); });",
      ...
      "          else if (inp.type === 'date') data.preferred_date = data.preferred_date || val;",
      "          else if (inp.type === 'time') data.preferred_time = data.preferred_time || val;",
      ...
      "          // A <select> on a lead form is almost always the thing the",
      "          // customer is asking for. Dropping it lost the chosen service",
      "          // on every site where this handler is the primary one.",
      "          else if (inp.tagName === 'SELECT' && !data.service_needed) data.service_needed = val;",
      "        });",
      "        // Detect form type from fields",
      "        const hasItems = !!data.items;",
      "        // A date field (or booking-flavored form) routes to the bookings API",
      "        const bookingDate = data.preferred_date || data.booking_date || data.date;",
      "        const looksLikeBooking = !!bookingDate && /^\\d{4}-\\d{2}-\\d{2}$/.test(bookingDate);",
      "        const endpoint = hasItems ? API_BASE + '/orders' : looksLikeBooking ? API_BASE + '/bookings' : API_BASE + '/submit-form';",
      "        // Add form_type if not present",
      "        if (!hasItems && !data.form_type) {",
      "          data.form_type = looksLikeBooking ? 'booking' : 'contact';",
    ...
    "    const CART_KEY = 'sitecraft_cart_' + projectId;",
    "    function getCart(): Array<{name:string;price:number;quantity:number;image?:string}> {",
    "      try { return JSON.parse(localStorage.getItem(CART_KEY) || '[]'); } catch { return []; }",
    "    }",
    "    function saveCart(c: Array<{name:string;price:number;quantity:number;image?:string}>) {",
    ...
    "        if (!(text.includes('add to cart') || text.includes('buy now'))) return;",
    "        if (btn.dataset.cartWired) return;",
    "        btn.dataset.cartWired = 'true';",
    "        btn.addEventListener('click', (e) => {",
    "          e.preventDefault();",
    "          e.stopPropagation();",
    "          // Try to find product info from nearby elements",
    "          const card = btn.closest('[class*=card], [class*=product], article, section');",
    ...
    "          if (existing) { existing.quantity += 1; } else { cart.push({ name, price, quantity: 1, image }); }",
    "          saveCart(cart);",
    ...
    "        if (!(text.includes('checkout') || text.includes('check out') || text.includes('proceed to pay'))) return;",
    "        if ((el as HTMLElement).dataset.checkoutWired) return;",
    "        (el as HTMLElement).dataset.checkoutWired = 'true';",
    ...
    "              method: 'POST',",
    "              headers: { 'Content-Type': 'application/json' },",
    "              body: JSON.stringify({ items: cart, successUrl: window.location.origin + '/?checkout=success', cancelUrl: window.location.href }),",
    "            });",
    "            const data = await res.json();",
    "            if (data.url) { saveCart([]); window.location.href = data.url; }",
    "            else { btn.textContent = data.error || 'Checkout unavailable'; setTimeout(() => { btn.textContent = origText; }, 3000); }",
    "          } catch { btn.textContent = 'Error - try again'; setTimeout(() => { btn.textContent = origText; }, 3000); }",
    "        });",
    "      });",
    "    }",
    "",
    ...
    "    const cartObserver = new MutationObserver(() => { wireCartButtons(); wireCheckoutButtons(); });",
    "    cartObserver.observe(document.body, { childList: true, subtree: true });",
    "",
      "    return () => { observer.disconnect(); cartObserver.disconnect(); };",
      "  }, [projectId]);",
      "",
      "  return null;",
      "}",
